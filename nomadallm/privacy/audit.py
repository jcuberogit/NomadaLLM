...
class AuditEventType(Enum):
    ...
    # Neuro-BCI events
    NEURO_STATE_CHANGE = "neuro_state_change"
    NEURO_EPOCH_REJECTED = "neuro_epoch_rejected"
...
class AuditLogger:
    ...
    def log_neuro_state(
        ...
        """Log a cognitive-state transition from the BCI stream.

        Security: Logs ONLY the state label, a hash of the band-power
        vector, and the epoch entropy. Never the raw signal.
        Compliance: Provides forensic traceability for neuro-AI decisions.
        """
        return self.log(
            event_type=AuditEventType.NEURO_STATE_CHANGE,
            action="neuro_state_change",
            user_id=user_id,
            session_id=session_id,
            resource="bci_stream",
            details={
                "cognitive_state": state,
                "band_power_hash": band_power_hash,
                "epoch_entropy": round(entropy, 4),
            },
            risk_level="low"
        )

    def log_neuro_epoch_rejected(
        self,
        user_id: str,
        reason: str,
        entropy: float,
        ...
        """Log a rejected epoch (noise / artifact).

        Security: Records rejection reason for forensic audit.
        """
        return self.log(
            event_type=AuditEventType.NEURO_EPOCH_REJECTED,
            action="neuro_epoch_rejected",
            user_id=user_id,
            session_id=session_id,
            resource="bci_stream",
            details={
